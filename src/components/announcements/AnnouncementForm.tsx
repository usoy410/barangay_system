...
import { Bell, ArrowLeft, Save, Loader2, Image as ImageIcon, Calendar } from 'lucide-react';
...
export function AnnouncementForm({ initialData, isEditing }: AnnouncementFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    starts_at: initialData?.starts_at || null as string | null,
    expires_at: initialData?.expires_at || null as string | null,
  });
          ...
          <div className="space-y-6">
            ...
            <div className="p-8 bg-slate-50 rounded-4xl border border-slate-100 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wider text-[10px]">Display Starts</label>
                  <input 
                    type="datetime-local" 
                    className="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition-all font-medium text-sm"
                  ...
                  />
                  <p className="mt-2 text-slate-400 text-[10px]">Leave blank to show immediately after publishing.</p>
                </div>

                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wider text-[10px]">Display Expires</label>
                  <input 
                    type="datetime-local" 
                    className="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 outline-none transition-all font-medium text-sm"
                  ...
                  />
                  <p className="mt-2 text-slate-400 text-[10px]">Leave blank to keep showing indefinitely.</p>
                </div>
              </div>
            </div>
          </div>
